 ...
 */
	...
	public static float serviceTime(){
		float ret = calXDistribution(i_oServTime);
		if(ret>.0009){
		return ret;
		}else{
			return q.nextFloat()/10;
		}
	}
	...
	public static float BurstLength(boolean isIO_Bound){
		Random rand = new Random();
		if(isIO_Bound){
		return ((float)rand.nextInt(20))/1000;
		//return ((float)((int)(((float)(rand.nextInt(20)+20))/100)))/100;
		}else{
		return ((float)rand.nextInt(20))/1000;
		//return ((float)((int)(((float)(rand.nextInt(100)+100))/100)))/100;
		}
	}
